...
seed(1337)
# max number of tfidf training documents (chosen randomly),
...
for pid, j in db.items():
    ...
    if os.path.isfile(txt_path):  # some pdfs dont translate to txt
        ...
        if len(txt) > 1000 and len(txt) < 500000:
            ...
            pids.append(idvv)
            print("read %d/%d (%s) with %d chars" % (n, len(db),
                                                     idvv, len(txt)))
        else:
            print("skipped %d/%d (%s) with %d chars: suspicious!" % (n,
                                                                     len(db),
                                                                     idvv,
                                                                     len(txt)))
    else:
        print("could not find %s in txt folder." % (txt_path, ))
print("in total read in %d text files out of %d db entries." % (len(txt_paths),
                                                                len(db)))
